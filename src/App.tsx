...
import { CommunicationWorkspace, DocumentsWorkspace, IntegrationsWorkspace, MeetingRecordingBanner, MeetingsWorkspace, OrganizationOverview, PeopleWorkspace, ProjectOverview, ProjectsWorkspace, VocabularyWorkspace, WorkspaceContextBar, WorkspaceNavigation, type WorkbenchArea } from "./workbench";
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace-main">
        ...
        {area === "project-overview" && activeProject && <ProjectOverview project={activeProject} workspace={workspace} tasks={tasks} onArea={setArea} onCapture={() => setComposerOpen(true)} />}
        ...
        {area === "communication" && activeProject && activeOrganization && <CommunicationWorkspace project={activeProject} organization={activeOrganization} workspace={workspace} onIntegrations={() => { setActiveProjectId(null); setArea("integrations"); }} onError={setError} />}
      ...
      {composerOpen && <Composer workspace={workspace} defaultProjectId={activeProject?.id ?? null} clockFormat={settings.clockFormat} audioInputMode={settings.audioInputMode} speechSettings={settings.speech} tomorrowReminderTime={settings.tomorrowReminderTime} transcribingAudioIds={transcribingAudioIds} onAudioTranscribing={setAudioTranscribing} onTaskProcessing={setTaskProcessing} onClose={() => setComposerOpen(false)} onSubmit={createTask} onTaskUpdate={updateTask} onError={setError} />}
...
function Composer({ workspace, defaultProjectId, clockFormat, audioInputMode, speechSettings, tomorrowReminderTime, transcribingAudioIds, onAudioTranscribing, onTaskProcessing, onClose, onSubmit, onTaskUpdate, onError }: { workspace: Workspace; defaultProjectId: string | null; clockFormat: AppSettings["clockFormat"]; audioInputMode: AudioInputMode; speechSettings: AppSettings["speech"]; tomorrowReminderTime: string; transcribingAudioIds: string[]; onAudioTranscribing: (id: string, active: boolean) => void; onTaskProcessing: (id: string, active: boolean) => void; onClose: () => void; onSubmit: (input: TaskInput) => Promise<Task>; onTaskUpdate: (id: string, patch: Partial<TaskInput>) => Promise<void>; onError: (error: string) => void }) {
  ...
  const [speechProvider, setSpeechProvider] = useState(speechSettings.provider);
  ...
  function startTranscription(result: RecorderResult, attachment: AudioAttachment): void {
    ...
    const job = api.transcribeWav(result.base64, undefined, undefined, speechProvider)
        .then((transcript) => {
  ...
  async function finishRecordingForTask(activeRecorder: WavRecorder, taskId: string): Promise<void> {
    ...
    try {
      ...
      const text = (await api.transcribeWav(result.base64, undefined, undefined, speechProvider)).trim();
      if (!text) return;
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form className="composer" onSubmit={submit}>
      ...
      <div className="audio-capture-options"><AudioInputSelect value={selectedAudioInput} disabled={recording || recordingBusy} onChange={setSelectedAudioInput} /><SpeechPrivacySelect value={speechProvider} disabled={recording || recordingBusy} onChange={setSpeechProvider} /></div>
      {speechProvider === "openai" && <p className="field-help privacy-disclosure">OpenAI cloud selected: this new audio recording will be uploaded for transcription.</p>}
...
function TaskDetails({ task, workspace, clockFormat, audioInputMode, speechSettings, tomorrowReminderTime, transcribingAudioIds, onAudioTranscribing, onTaskProcessing, onClose, onUpdate, onRestore, onDelete, onError }: { task: Task; workspace: Workspace; clockFormat: AppSettings["clockFormat"]; audioInputMode: AudioInputMode; speechSettings: AppSettings["speech"]; tomorrowReminderTime: string; transcribingAudioIds: string[]; onAudioTranscribing: (id: string, active: boolean) => void; onTaskProcessing: (id: string, active: boolean) => void; onClose: () => void; onUpdate: (patch: Partial<TaskInput>) => Promise<void>; onRestore: () => Promise<void>; onDelete: () => Promise<void>; onError: (error: string) => void }) {
  ...
  async function toggleRecording() {
    ...
    try {
      ...
      onAudioTranscribing(attachment.id, true);
      void api.transcribeWav(result.base64, undefined, undefined, speechProvider).then(async (transcript) => {
        const text = transcript.trim();
        if (!text) return;
        const latestTask = (await api.listTasks()).find((item) => item.id === task.id);
...
function SpeechPrivacySelect({ value, disabled, compact = false, onChange }: { value: AppSettings["speech"]["provider"]; disabled: boolean; compact?: boolean; onChange: (provider: AppSettings["speech"]["provider"]) => void }) {
  const [cloudConfigured, setCloudConfigured] = useState<boolean | null>(null);
  useEffect(() => {
    void api.speechCloudStatus().then((status) => {
      setCloudConfigured(status.configured);
      if (!status.configured && value === "openai") onChange("local");
    }).catch(() => {
      setCloudConfigured(false);
      if (value === "openai") onChange("local");
    });
  }, []);
  return <label className={`audio-input-select speech-privacy-select ${compact ? "compact" : ""}`}><span>Transcription</span><select value={value} disabled={disabled || cloudConfigured === null} onChange={(event) => onChange(event.target.value as AppSettings["speech"]["provider"])}><option value="local">Local - private</option><option value="openai" disabled={!cloudConfigured}>OpenAI cloud</option></select></label>;
}
