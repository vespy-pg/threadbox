...
export function CommunicationWorkspace({ project, organization, workspace, onIntegrations, onError }: { project: Project; organization: Organization; workspace: Workspace; onIntegrations: () => void; onError: (message: string) => void }) {
  ...
  const loadFoundation = useCallback(async () => {
    try {
      ...
      const mailConnections = nextConnections.filter((item) => ["google", "microsoft", "standard_mail"].includes(item.connection.provider));
      setConnections(mailConnections);
      setConnectionId((current) => mailConnections.some((item) => item.connection.id === current) ? current : mailConnections[0]?.connection.id ?? "");
  ...
  function preparePreview() {
    ...
    setPreview({ organizationId: organization.id, projectId: project.id, connectionId, to: recipients, cc: [], bcc: [], subject: subject.trim(), body, threadId: replyContext?.threadId ?? null, inReplyTo: replyContext?.messageId || null, references: replyContext ? [replyContext.references, replyContext.messageId].filter(Boolean).join(" ") || null : null, replyToExternalId: replyContext?.id ?? null });
  }
  async function executeMail(sendNow: boolean) {
    ...
    try {
      ...
      const providerName = selectedConnection?.connection.provider === "microsoft" ? "Microsoft 365" : selectedConnection?.connection.provider === "standard_mail" ? "SMTP" : "Gmail";
      setNotice(sendNow ? `Email accepted by ${providerName} (${result.id}).` : `Draft created in ${providerName} (${result.draftId ?? result.id}).`);
...
export function IntegrationsWorkspace({ organization, onError }: { organization: Organization; onError: (message: string) => void }) {
  ...
  async function saveMicrosoftClientId() {
    if (!settings) return;
    try { setBusy("microsoft-client-id"); setSettings(await api.updateSettings({ ...settings, microsoftOauthClientId: microsoftClientId.trim() })); }
    catch (reason) { onError(String(reason)); }
    finally { setBusy(null); }
  }
  ...
  async function connectMicrosoft(connectionId?: string) {
    try { setBusy(connectionId ?? "connect-microsoft"); await api.connectMicrosoft(organization.id, connectionId); await load(); }
    catch (reason) { onError(String(reason)); }
    finally { setBusy(null); }
  }
  async function changeMicrosoftCapability(connectionId: string, capability: IntegrationCapabilityId, granted: boolean) {
    try { setBusy(`${connectionId}:${capability}`); if (granted) await api.grantMicrosoftCapability(connectionId, capability); else await api.revokeMicrosoftCapability(connectionId, capability); await load(); }
    catch (reason) { onError(String(reason)); }
    finally { setBusy(null); }
  }
  async function disconnectMicrosoft(connectionId: string) {
    if (!window.confirm("Disconnect this Microsoft account and delete its stored token? Imported project email remains in Threadbox.")) return;
    try { setBusy(connectionId); await api.disconnectMicrosoft(connectionId); await load(); }
    catch (reason) { onError(String(reason)); }
    finally { setBusy(null); }
  }
  const integrations = [
  ...
  return <WorkspacePage eyebrow="Organisation" title="Integrations" description={`Connections available to ${organization.name}. Each integration clearly states what it reads, creates and retains.`}>
    <section className="surface-card google-integration"><div className="surface-card-heading"><div><p className="eyebrow">Calendar and email</p><h2>Google Workspace</h2><p>Connect an account once, then enable only the calendar and Gmail outcomes you need.</p></div><span className="status-pill ready">Available</span></div>
      ...
      <div className="google-connections">{connections.filter((item) => item.connection.provider === "google").map((snapshot) => <ProviderConnectionCard key={snapshot.connection.id} snapshot={snapshot} capabilities={googleCapabilities} busy={busy} onReconnect={() => void connect(snapshot.connection.id)} onCapability={(capability, granted) => void changeCapability(snapshot.connection.id, capability, granted)} onDisconnect={() => void disconnect(snapshot.connection.id)} />)}
    ...
    <section className="surface-card google-integration"><div className="surface-card-heading"><div><p className="eyebrow">Email</p><h2>Microsoft 365 and Outlook</h2><p>Use delegated Microsoft Graph permissions for bounded mailbox access, drafts and reviewed sending.</p></div><span className="status-pill ready">Available</span></div>
      <details className="oauth-client-settings" open={!settings?.microsoftOauthClientId}><summary>Microsoft app configuration</summary><p>Development builds need a public desktop application client ID with a localhost redirect URI. Release builds will include the verified Threadbox registration.</p><div><input value={microsoftClientId} onChange={(event) => setMicrosoftClientId(event.target.value)} placeholder="00000000-0000-0000-0000-000000000000" /><button className="secondary-button" disabled={!settings || !microsoftClientId.trim() || busy === "microsoft-client-id"} onClick={() => void saveMicrosoftClientId()}>Save client ID</button></div></details>
      <div className="google-connections">{connections.filter((item) => item.connection.provider === "microsoft").map((snapshot) => <ProviderConnectionCard key={snapshot.connection.id} snapshot={snapshot} capabilities={mailCapabilities} busy={busy} onReconnect={() => void connectMicrosoft(snapshot.connection.id)} onCapability={(capability, granted) => void changeMicrosoftCapability(snapshot.connection.id, capability, granted)} onDisconnect={() => void disconnectMicrosoft(snapshot.connection.id)} />)}<button className="secondary-button google-connect" disabled={!settings?.microsoftOauthClientId || busy !== null} onClick={() => void connectMicrosoft()}><Plus size={15} />{busy === "connect-microsoft" ? "Finish in browser..." : "Connect Microsoft account"}</button></div>
...
const mailCapabilities = googleCapabilities.filter((capability) => capability.id.startsWith("mail_"));

function ProviderConnectionCard({ snapshot, capabilities, busy, onReconnect, onCapability, onDisconnect }: { snapshot: IntegrationSnapshot; capabilities: typeof googleCapabilities; busy: string | null; onReconnect: () => void; onCapability: (capability: IntegrationCapabilityId, granted: boolean) => void; onDisconnect: () => void }) {
  ...
  return <article className="google-account-card"><header><div><strong>{snapshot.connection.displayName}</strong><small>{snapshot.connection.accountIdentifier}</small></div><span className="model-ready"><CheckCircle2 size={14} />Connected</span></header><div className="capability-list">{capabilities.map((capability) => <div key={capability.id}><span><strong>{capability.title}</strong><small>{capability.text}</small></span><button className={granted(capability.id) ? "text-button danger" : "secondary-button"} disabled={busy !== null} onClick={() => onCapability(capability.id, !granted(capability.id))}>{busy === `${snapshot.connection.id}:${capability.id}` ? "Finish in browser..." : granted(capability.id) ? "Revoke" : "Grant"}</button></div>)}</div><footer><button className="text-button" disabled={busy !== null} onClick={onReconnect}>Repair access</button><button className="text-button danger" disabled={busy !== null} onClick={onDisconnect}>Disconnect</button></footer></article>;
}

function StandardMailSetup({ organization, connections, onConnected, onError }: { organization: Organization; connections: IntegrationSnapshot[]; onConnected: () => Promise<void>; onError: (message: string) => void }) {
  ...
  const [smtpSecurity, setSmtpSecurity] = useState<"tls" | "starttls">("starttls");
  const [smtpPassword, setSmtpPassword] = useState("");
  const [busy, setBusy] = useState(false);
  const [diagnostics, setDiagnostics] = useState<Record<string, string>>( {} );
  function preset(kind: "fastmail" | "proton" | "manual") {
    if (kind === "fastmail") { setImapHost("imap.fastmail.com"); setImapPort(993); setSmtpHost("smtp.fastmail.com"); setSmtpPort(465); setSmtpSecurity("tls"); }
    if (kind === "proton") { setImapHost("127.0.0.1"); setImapPort(1143); setSmtpHost("127.0.0.1"); setSmtpPort(1025); setSmtpSecurity("starttls"); }
    if (kind === "manual") { setImapHost(""); setSmtpHost(""); }
  }
  async function save() {
    setBusy(true);
    try { await api.connectStandardMail({ organizationId: organization.id, email: email.trim(), displayName: email.trim(), username: username.trim() || email.trim(), imapHost: imapHost.trim() || null, imapPort: imapHost.trim() ? imapPort : null, imapPassword: imapHost.trim() ? imapPassword : null, smtpHost: smtpHost.trim() || null, smtpPort: smtpHost.trim() ? smtpPort : null, smtpSecurity: smtpHost.trim() ? smtpSecurity : null, smtpPassword: smtpHost.trim() ? smtpPassword : null }); setShow(false); setImapPassword(""); setSmtpPassword(""); await onConnected(); }
    catch (reason) { onError(String(reason)); }
    finally { setBusy(false); }
  }
  async function diagnose(id: string) { setBusy(true); try { const result = await api.diagnoseStandardMail(id); setDiagnostics((current) => ({ ...current, [id]: `Incoming: ${result.incomingMessage}. Outgoing: ${result.outgoingMessage}.` })); } catch (reason) { onError(String(reason)); } finally { setBusy(false); } }
  async function disconnect(id: string) { if (!window.confirm("Disconnect this mail account and delete both stored passwords?")) return; try { await api.disconnectStandardMail(id); await onConnected(); } catch (reason) { onError(String(reason)); } }
  return <section className="surface-card google-integration"><div className="surface-card-heading"><div><p className="eyebrow">Other email providers</p><h2>IMAP and SMTP</h2><p>Incoming and outgoing access are configured independently. Only TLS and required STARTTLS are accepted.</p></div><button className="secondary-button" onClick={() => setShow((value) => !value)}><Plus size={14} />Add account</button></div>
    {show && <div className="standard-mail-form"><div className="channel-picker"><button onClick={() => preset("fastmail")}>Fastmail</button><button onClick={() => preset("proton")}>Proton Bridge</button><button onClick={() => preset("manual")}>Manual</button></div><label><span>Email</span><input value={email} onChange={(event) => { setEmail(event.target.value); if (!username) setUsername(event.target.value); }} /></label><label><span>Username</span><input value={username} onChange={(event) => setUsername(event.target.value)} /></label><fieldset><legend>Incoming - optional</legend><input value={imapHost} onChange={(event) => setImapHost(event.target.value)} placeholder="IMAP host" /><input type="number" value={imapPort} onChange={(event) => setImapPort(Number(event.target.value))} /><input type="password" value={imapPassword} onChange={(event) => setImapPassword(event.target.value)} placeholder="Password or app password" /></fieldset><fieldset><legend>Outgoing - optional</legend><input value={smtpHost} onChange={(event) => setSmtpHost(event.target.value)} placeholder="SMTP host" /><input type="number" value={smtpPort} onChange={(event) => setSmtpPort(Number(event.target.value))} /><select value={smtpSecurity} onChange={(event) => setSmtpSecurity(event.target.value as "tls" | "starttls")}><option value="starttls">Required STARTTLS</option><option value="tls">Implicit TLS</option></select><input type="password" value={smtpPassword} onChange={(event) => setSmtpPassword(event.target.value)} placeholder="Password or app password" /></fieldset><div className="communication-actions"><button className="primary-button" disabled={busy || !email.trim() || (!imapHost.trim() && !smtpHost.trim())} onClick={() => void save()}>Store in keyring and connect</button></div></div>}
    <div className="google-connections">{connections.map((snapshot) => <article className="google-account-card" key={snapshot.connection.id}><header><div><strong>{snapshot.connection.displayName}</strong><small>{snapshot.connection.accountIdentifier}</small></div><span className="model-ready"><CheckCircle2 size={14} />Connected</span></header>{diagnostics[snapshot.connection.id] && <p className="integration-permission-note">{diagnostics[snapshot.connection.id]}</p>}<footer><button className="text-button" disabled={busy} onClick={() => void diagnose(snapshot.connection.id)}>Test incoming and outgoing</button><button className="text-button danger" onClick={() => void disconnect(snapshot.connection.id)}>Disconnect</button></footer></article>)}</div>
  </section>;
}
